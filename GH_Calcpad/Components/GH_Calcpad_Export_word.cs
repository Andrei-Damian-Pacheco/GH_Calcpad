...
{
    /// <summary>
    /// Exports Calcpad's rendered report to a native .docx via Calcpad.OpenXml (same writer
    /// Calcpad's own CLI uses) - real OMath equations, no Word installation or COM automation.
    /// </summary>
    public class GH_Calcpad_Export_word : GH_Component
    {
        public GH_Calcpad_Export_word()
          : base("Export Word", "ExportWord",
                 "Exports Calcpad's rendered report to a native .docx",
        ...
        protected override void RegisterOutputParams(GH_OutputParamManager p)
        {
            ...
            p.AddBooleanParameter("Success", "OK", "True if .docx was generated", GH_ParamAccess.item);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            var (success, finalPath, error) = CalcpadExporter.Export(sheet, outputFolder, fileName, "docx");

            DA.SetData(0, success ? finalPath : null);
            DA.SetData(1, success);

            if (success)
                AddRuntimeMessage(GH_RuntimeMessageLevel.Remark, $"DOCX export -> {Path.GetFileName(finalPath)}");
            else
                AddRuntimeMessage(GH_RuntimeMessageLevel.Error, $"DOCX export failed: {error}");
        }
