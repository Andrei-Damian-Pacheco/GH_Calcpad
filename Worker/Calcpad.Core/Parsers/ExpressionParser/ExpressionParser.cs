                ...
                if (token.Type == TokenTypes.Expression)
                {
                    try
                    {
                        ...
                        if (_calculate && _isVal > -1)
                        {
                            ...
                            // Record every assignment regardless of isOutput/visibility - Calculate()
                            // always evaluates and assigns the real value+unit even inside a #hide
                            // block (isOutput only gates whether the line gets printed to HTML). For
                            // GH_Calcpad's JSON worker there is no HTML report, so a variable set
                            // inside #hide must still be capturable (e.g. a ghc_ objective kept out
                            // of a printed report).
                            if (_parser.TryGetLastAssignment(out var raName, out var raValue, out var raUnit))
                                _resultRecords.Add(new ResultRecord(raName, raValue, raUnit));
                        }
                        else
